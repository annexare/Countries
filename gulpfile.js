...
exports[DO_COPY] = function copy(callback) {
  fs.writeFileSync(
    ...
    JSON.stringify(continents, false, JSON_TAB) + LF
  )
  fs.writeFileSync(
    `${DIST}${COUNTRIES}.${JSON_EXT}`,
    JSON.stringify(countries, false, JSON_TAB) + LF
  )
  fs.writeFileSync(
    `${DIST}${LANGUAGES}.${JSON_EXT}`,
    JSON.stringify(languagesInUse, false, JSON_TAB) + LF
  )
  fs.writeFileSync(
    `${DIST}${LANGUAGES}.${ALL}.${JSON_EXT}`,
    JSON.stringify(languages, false, JSON_TAB) + LF
  )
  callback && callback()
}
  ...
  const csvData =
    ...
    countryList
      .map((code) => {
        ...
        return (
          QUOTE + code + COMMA + objectValues(getCountryDataOrdered(country)).join(COMMA) + QUOTE
        )
      })
      .join(LF)
...
exports[DO_MIN] = function min(callback) {
  ...
  fs.writeFileSync(
    `${DIST}${LANGUAGES}.${ALL}.${DO_MIN}.${JSON_EXT}`,
    JSON.stringify(languages) + LF
  )
  callback && callback()
}
